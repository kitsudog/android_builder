...
{
    if( ndkHelper::GLContext::getInstance()->getGLVersion() >= 3.0 )
    {
        _bGeometryInstancingSupport = true;
    }
    else if( ndkHelper::GLContext::getInstance()->checkExtension("GL_NV_draw_instanced")
            && ndkHelper::GLContext::getInstance()->checkExtension("GL_NV_uniform_buffer_object") )
    {
    ...
    for( int32_t iX = 0; iX < _iX; ++iX )
        for( int32_t iY = 0; iY < _iY; ++iY )
            ...
            {
                _mModels.push_back( ndkHelper::mat4::translation( iX * fGapX + fOffsetX,
                        iY * fGapY + fOffsetY,
                        iZ * fGapZ + fOffsetZ) );
                _mColors.push_back(
                        ndkHelper::vec3( random() / float(RAND_MAX * 1.1),
                ...
                _mVecRotation.push_back(
                        ndkHelper::vec2( fX * 0.05f, fY * 0.05f ) );
                _mCurrentRotation.push_back(
                        ndkHelper::vec2( fX * M_PI, fY * M_PI ) );
            }
...
void MoreTeapotsRenderer::updateViewport()
{
    ...
    bool bRotate = false;
    _mProjection = ndkHelper::mat4::perspective(fAspect, 1.f,
            CAM_NEAR, CAM_FAR);
}
...
void MoreTeapotsRenderer::update(float fTime)
{
    ...
    _mView = ndkHelper::mat4::lookAt(ndkHelper::vec3(CAM_X, CAM_Y, CAM_Z),
            ndkHelper::vec3(0.f, 0.f, 0.f), ndkHelper::vec3(0.f, 1.f, 0.f));
...
void MoreTeapotsRenderer::render()
{
    ...
    if( _bGeometryInstancingSupport )
    {
        ...
        for( int32_t i = 0; i < _iX * _iY * _iZ; ++i )
        {
            ...
            ndkHelper::mat4 mRotation = ndkHelper::mat4::rotationX( fX ) * ndkHelper::mat4::rotationY( fY );

            // Feed Projection and Model View matrices to the shaders
            ndkHelper::mat4 mV = _mView * _mModels[ i ] * mRotation;
            ndkHelper::mat4 mVP = _mProjection * mV;
...
bool MoreTeapotsRenderer::loadShaders(SHADER_PARAMS* params, const char* strVsh, const char* strFsh)
{
    ...
    if (!ndkHelper::shader::compileShader(&vertShader, GL_VERTEX_SHADER, strVsh)) {
    ...
    if (!ndkHelper::shader::compileShader(&fragShader, GL_FRAGMENT_SHADER, strFsh)) {
    ...
    // Link program
    if (!ndkHelper::shader::linkProgram(program)) {
        ...
        std::map<std::string, std::string>&shaderParams )
{
    ...
    // Create and compile vertex shader
    if (!ndkHelper::shader::compileShader(&vertShader, GL_VERTEX_SHADER, strVsh, shaderParams)) {
        LOGI("Failed to compile vertex shader");
        glDeleteProgram(program);
        return false;
    }
    ...
    // Create and compile fragment shader
    if (!ndkHelper::shader::compileShader(&fragShader, GL_FRAGMENT_SHADER, strFsh, shaderParams)) {
        LOGI("Failed to compile fragment shader");
        glDeleteProgram(program);
        return false;
    }
...
// Bind
//--------------------------------------------------------------------------------
bool MoreTeapotsRenderer::bind(ndkHelper::tapCamera* camera)
{
    _camera = camera;
    return true;
}
